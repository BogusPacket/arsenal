...
public:
  UDP(){server.sin_family=AF_INET;
        ...
        template <class Buf> void RECV(Buf buf, size_t size){socklen_t* fromlen = sizeof(sockaddr); ssize_t i = recvfrom(this->sock, buf, size, 0, (sockaddr*)&this->server, &fromlen);}
};
